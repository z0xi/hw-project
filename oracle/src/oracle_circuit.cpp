...
// OracleOnlineProtocol::OracleOnlineProtocol(void){}

share* 
OracleOnlineProtocol::BuildDivCircuit(share* dividend, share* divisor, uint32_t nvals, BooleanCircuit* circ) {
...
share* 
OracleOnlineProtocol::BuildInverseRandomCircuit(share* msg, share* divRand, share* subRand, uint32_t bufSize, uint32_t nvals, BooleanCircuit* circ){
	uint32_t zero = 0;
	uint32_t one = 0;
	uint32_t bitlen_8 = 8;
...
bool 
OracleOnlineProtocol::runProtocolCircuit(std::string signature_base64, uint32_t bufSize, e_role role, const std::string& address, uint16_t port, seclvl seclvl, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing) {
	uint32_t bitlen_8 = 8;
	...
	for(int i = 0 ;i < ABY_SHA256_OUTPUT_BYTES; i++)
	verify.SetByte(i,result.c_str()[i]);
	delete party;
	bool success= 0;
...
share* 
OracleOnlineProtocol::BuildSHA256Circuit(share* s_msgInput, uint32_t nvals, uint32_t bufSize, uint64_t msgSize, BooleanCircuit* circ) {
...
share* 
OracleOnlineProtocol::BuildSHA1Circuit(share* s_msgInput, uint8_t* msgInput, uint8_t* plain_out, uint32_t nvals, BooleanCircuit* circ) {
...
void 
OracleOnlineProtocol::init_variables(share** s_h, uint32_t* h, uint32_t nvals, BooleanCircuit* circ) {
...
void 
OracleOnlineProtocol::init_variables(share** s_h, uint32_t nvals, BooleanCircuit* circ) {
...
void 
OracleOnlineProtocol::init_AH(share** s_h, uint32_t* h, uint32_t nvals, BooleanCircuit* circ) {
...
share* 
OracleOnlineProtocol::process_block(share* s_msg, share** s_h, uint32_t nvals, BooleanCircuit* circ) {
...
share* 
OracleOnlineProtocol::process_block(share* s_msg, uint8_t* msg, uint8_t* plain_out, share** s_h, uint32_t* h, uint32_t nvals, BooleanCircuit* circ) {
...
void 
OracleOnlineProtocol::break_message_to_chunks(share** s_w, share* s_msg, BooleanCircuit* circ) {
...
void 
OracleOnlineProtocol::break_message_to_chunks(share** s_w, share* s_msg, uint32_t* w, uint8_t* msg, BooleanCircuit* circ) {
	for(uint32_t i = 0; i < 16; i++) {
		s_w[i] = new boolshare(32, circ);
	}
	//iterate over message bytes
...
//for i from 16 to 79
 //   w[i] = (w[i-3] xor w[i-8] xor w[i-14] xor w[i-16]) leftrotate 1
void 
OracleOnlineProtocol::expand_ws(share** s_w, uint32_t* w, BooleanCircuit* circ) {
	share* s_wtmp;
...
//For sha256
void 
OracleOnlineProtocol::expand_ws(share** s_w, BooleanCircuit* circ) {
...
share* 
OracleOnlineProtocol::ch(share* s_e, share* s_f, share* s_g, BooleanCircuit* circ) {
	share *out, *temp;
...
share* 
OracleOnlineProtocol::sigma1(share* s_e, BooleanCircuit* circ) {
	share *out, *temp;
...
share* 
OracleOnlineProtocol::ma(share* s_a, share* s_b, share* s_c, BooleanCircuit* circ) {
	share *out, *temp;
...
share* 
OracleOnlineProtocol::sigma0(share* s_a, BooleanCircuit* circ) {
	share *out, *temp;
...
void 
OracleOnlineProtocol::sha256_main_loop(share** s_hash, share** s_w, uint32_t nvals, BooleanCircuit* circ) {
	/*
...
void 
OracleOnlineProtocol::sha1_main_loop(share** s_h, share** s_w, uint32_t* h, uint32_t* w, uint32_t nvals, BooleanCircuit* circ) {
	/*
	 * Initialize hash value for this chunk:
	 * a = h0; b = h1; c = h2; d = h3; e = h4
	*/
...
void 
OracleOnlineProtocol::verify_SHA1_hash(uint8_t* msg, uint32_t msgbytes, uint32_t nvals, uint8_t* hash) {
...
void 
OracleOnlineProtocol::verify_SHA256_hash(uint8_t* msg, uint32_t msgbytes, uint32_t nvals, uint8_t* hash) {
