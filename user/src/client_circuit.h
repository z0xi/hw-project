#ifndef __CLIENT_CIRCUIT_H_
#define __CLIENT_CIRCUIT_H_
...
class ClientOnlineProtocol
{
    public:
        // ClientOnlineProtocol();
        uint16_t* msgArray;
        bool runProtocolCircuit(uint32_t bufSize, e_role role, const std::string& address, uint16_t port, seclvl seclvl, uint32_t nvals, uint32_t nthreads, e_mt_gen_alg mt_alg, e_sharing sharing);
